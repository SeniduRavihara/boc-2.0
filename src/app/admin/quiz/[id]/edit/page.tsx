...
export default function EditQuizPage() {
  ...
  const [registrationTime, setRegistrationTime] = useState(2);
  ...
  const fetchQuiz = async () => {
    try {
      ...
      if (quiz) {
        ...
        setRegistrationTime(quiz.registrationTime);
        setDefaultQuestionTime(quiz.defaultQuestionTime);
        setMode(quiz.mode);
    ...
    <div className="min-h-screen bg-[#030712] text-slate-200 p-6 md:p-12 font-sans overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <div className="mb-12">
          ...
          <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">Edit Quiz</h1>
        </div>

        <form onSubmit={handleSubmit} className="space-y-12 pb-24">
          <GlassCard className="p-8 border-slate-800/50">
            <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
              <Settings className="text-purple-400" size={20} /> Quiz Settings
            </h2>
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-2">
                    <Clock size={14} className="text-blue-400" /> Registration Time (Mins)
                  </label>
                  <input 
                    type="number" 
                    value={registrationTime}
                    onChange={e => setRegistrationTime(parseInt(e.target.value) || 2)}
                    min={1}
                    className="w-full bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-white outline-none focus:border-purple-500/50 transition-all font-medium"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-2">
                    <Zap size={14} className="text-yellow-400" /> Question Time (Secs)
                  </label>
                  <input 
                    type="number" 
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Quiz Mode</label>
                <div className="flex gap-4">
                  ...
                  >
                    <Zap size={20} />
                    <span className="font-bold text-sm">Automatic Mode</span>
                  </button>
                </div>
              </div>
            </div>
                  ...
                  <GlassCard className="p-8 border-slate-800/50 relative">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <input 
                        ...
                        className="w-24 bg-slate-950/50 border border-slate-800 rounded-lg p-2 text-sm text-center outline-none text-white font-mono"
                      />
                    </div>
                  </GlassCard>
                </motion.div>
